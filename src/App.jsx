    ...
    <tr ref={setNodeRef} style={style} className="group hover:bg-white/[0.02] transition-colors bg-[#1a1a1a]">
      ...
      <td className="py-4 px-2 hidden md:table-cell">
        <div className="flex items-center gap-6 justify-end">
          {/* One Calendar Icon + DD/MM/YYYY Text */}
    ...
    <div className="min-h-screen w-full bg-[#1a1a1a] text-slate-300 p-4 md:p-14 font-sans select-none relative">
      ...
      {activeTab === 'tasks' && (
        <div className="fixed bottom-8 right-8 flex gap-3 z-50">
          ...
          <button onClick={async () => {const name=prompt("Task?"); if(name) await supabase.from('tasks').insert([{name, status:'To Do', priority:'MEDIUM', deadline:new Date().toLocaleDateString('en-CA'), position:tasks.length}]); fetchData();}} className="bg-blue-600 p-4 rounded-2xl shadow-2xl active:scale-95 transition-all"><Plus size={24} className="text-white" /></button>
        </div>
      )}
    </div>
  );
