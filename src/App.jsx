  ...
  return (
    <tr ref={setNodeRef} style={style} className="group hover:bg-white/[0.02] transition-colors bg-[#1a1a1a]">
      ...
      <td className="py-4 px-2 md:px-4">
        <input 
          className={`bg-transparent border-none outline-none text-lg md:text-xl font-bold w-full ${isDone ? 'line-through text-slate-700' : 'text-slate-200 focus:text-blue-500'}`}
          value={t.name}
          onChange={(e) => updateTaskField(t.id, 'name', e.target.value)}
        />
      </td>

      {/* HIDDEN ON PHONE - PREVENTS OVERLAP */}
      ...
      <td className="py-4 px-2 md:px-4">
        <button 
          ...
          className={`mx-auto block w-24 md:w-36 py-2 rounded-full border-2 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-center transition-all ${
      ...
      <td className="py-4 px-4 hidden md:table-cell">
        <div className={`flex items-center gap-2 font-bold text-xs uppercase italic ${time.color}`}>
      ...
      <td className="py-4 px-2 md:px-4">
        <input 
          ...
          className="bg-transparent border border-white/5 rounded px-2 py-1 text-[10px] md:text-xs text-slate-400 font-bold [color-scheme:dark]"
        />
      </td>

      <td className="py-4 px-2 text-right">
        <button onClick={() => deleteTask(t.id)} className="p-2 text-slate-800 hover:text-red-500 opacity-0 group-hover:opacity-100">
          <Trash2 size={20} />
        </button>
      </td>
    </tr>
...
export default function App() {
  ...
  // AUTO-SORT LOGIC: Urgent -> High -> Med -> Low, then by Date
  ...
  const getTimeLeft = (deadline, isDone) => {
    ...
    if (diff < 0) return { text: "OVERDUE", color: "text-red-600 font-black animate-pulse" };
    if (diff === 0) return { text: "TODAY", color: "text-red-400" };
    if (diff < 5) return { text: `${diff} DAYS`, color: "text-orange-400" };
    return { text: `${diff} DAYS`, color: "text-emerald-500" };
  };
  ...
  if (loading) return <div className="bg-[#1a1a1a] min-h-screen text-white p-10 font-black uppercase italic">Syncing...</div>;
  ...
  return (
    <div className="min-h-screen w-full bg-[#1a1a1a] text-slate-300 p-4 md:p-14 font-sans select-none">
      <div className="w-full mb-12 flex justify-between items-center border-b border-white/5 pb-8">
        ...
        <button onClick={addTask} className="bg-blue-600 text-white p-4 md:p-5 rounded-2xl shadow-lg shadow-blue-600/20 active:scale-90 transition-transform">
          <Plus size={28} md:size={32} strokeWidth={4} />
        </button>
      </div>
          ...
          <table className="w-full text-left border-separate border-spacing-y-2 table-fixed">
            <thead>
              <tr className="text-slate-700 text-[10px] font-black uppercase tracking-[0.25em]">
                <th className="w-10 md:w-16"></th>
                <th className="w-12 md:w-20"></th>
                <th className="text-slate-500">TASKS</th>
                <th className="w-32 hidden md:table-cell">STATUS</th>
                <th className="w-28 md:w-44 text-center">PRIORITY</th>
                <th className="w-32 hidden md:table-cell">TIME</th>
                <th className="w-28 md:w-48 text-center">DATE</th>
