...
export default function App() {
  ...
  const readinessData = useMemo(() =>
    subjects.map(s => ({ name: s.name, percentage: Math.round((s.completed_modules / s.total_modules) * 100) || 0 })),
  [subjects]);
  ...
  const addTask = async () => {
    ...
    if (!name) return;
    const { data } = await supabase.from('tasks').insert([{ name, status: 'To Do', priority: 'MEDIUM', deadline: new Date().toLocaleDateString('en-CA') }]).select();
    if (data) setTasks(prev => [...prev, data[0]]);
  };
          ...
          <table className="w-full text-left border-separate border-spacing-y-2 table-fixed min-w-[600px] md:min-w-full">
            <thead>
              <tr className="text-slate-700 text-[10px] font-black uppercase tracking-[0.25em]">
                ...
                <th className="w-12 px-2 pb-4"></th>
                <th className="px-2 pb-4 text-slate-500">TASKS</th>
                <th className="w-32 hidden md:table-cell px-4 pb-4 text-slate-500">STATUS</th>
                <th className="w-32 md:w-44 text-center px-2 pb-4 text-slate-500">PRIORITY</th>
                <th className="w-32 hidden md:table-cell px-4 pb-4 text-slate-500">TIME</th>
                <th className="w-32 md:w-48 text-center px-2 pb-4 text-slate-500">DATE</th>
                <th className="w-10 px-2 pb-4"></th>
              </tr>
            </thead>
            ...
            {subjects.map(s => (
              <div key={s.id} className="bg-[#1a1a1a] p-5 rounded-2xl border border-white/5 mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  <input type="number" value={s.completed_modules} onChange={(e) => supabase.from('subjects').update({ completed_modules: parseInt(e.target.value) }).eq('id', s.id).then(() => fetchData())} className="bg-white/5 w-16 text-center py-1 rounded-lg font-bold text-purple-400" />
                  <span className="text-slate-700">/</span>
                  <input type="number" value={s.total_modules} onChange={(e) => supabase.from('subjects').update({ total_modules: parseInt(e.target.value) }).eq('id', s.id).then(() => fetchData())} className="bg-white/5 w-16 text-center py-1 rounded-lg font-bold text-slate-400" />
                </div>
              </div>
            ))}
          </div>
